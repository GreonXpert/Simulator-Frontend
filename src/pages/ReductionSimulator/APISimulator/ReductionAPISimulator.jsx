...
const ReductionAPISimulator = () => {
  ...
  liveRef.current = {
    baseUrl, clientId, projectId, calculationMethodology, apiKey, apiEndpoint,
  ...
  const buildPayload = (overrideIdx = null) => {
    ...
    const sourceRef = L.apiEndpoint.trim() ? { apiEndpoint: L.apiEndpoint.trim() } : {};
    ...
    if (L.calculationMethodology === "methodology1") {
      ...
      if (L.m1Mode === "random") {
        ...
        const idx = overrideIdx !== null ? overrideIdx : L.sendsCompleted;
        if (idx === L.distributions.totalIntervals - 1) {
    ...
    if (L.calculationMethodology === "methodology2") {
      ...
      return { ...base, variables, ...sourceRef };
    }
  ...
  const sendOnce = async () => {
    ...
    if (!L.apiKey) {
      log("❌ NET_API key required — enter key in Credential Secret", "err"); return;
    }

    // Build URL: serverAddress + fixed /api/net-reduction/ prefix + path params
    const url = `${L.baseUrl}/api/net-reduction/${L.clientId}/${L.projectId}/${L.calculationMethodology}/api`;
    ...
    try {
      log(`POST → Packet [${L.calculationMethodology}]`, "info");
      ...
      if (res.ok) {
        ...
        if (res.status === 202) {
          ...
          log(`✓ Net Reduction saved`, "ok");
        }
      } else {
        const errBody = await res.json().catch(() => ({}));
        log(`✖ Fail: ${res.status}${errBody.message ? ' — ' + errBody.message : ''}`, "err");
      }
    } catch { log(`✖ Connection error — check server address`, "err"); }
  };
  ...
  const startAuto = () => {
    ...
    const ms = L.targetFrequency === "Minutes"
      ...
      : Math.max(300, parseInt(L.intervalMs || 15000, 10));
    setStatus("RUNNING");
  ...
  const stopAuto = () => {
    if (timerRef.current) {
      ...
      setStatus("STOPPED"); log("Broadcaster halted", "warn");
    }
  };
  ...
  const styles = {
    ...
    rowItem: { display: "flex", gap: "8px", alignItems: "center", marginBottom: "10px" },
  ...
  const builtUrl = `${baseUrl}/api/net-reduction/${clientId || "•"}/${projectId || "•"}/${calculationMethodology}/api`;
          ...
          <div className="main">
            ...
            <div style={styles.card}>
              ...
              <div style={styles.fixedPath}>
                Fixed path: /api/net-reduction/<span style={{ color: "#6B7280" }}>:clientId/:projectId/:methodology/api</span>
              </div>
              ...
              <div style={styles.grid2}>
                ...
                <div>
                  <label style={styles.label}>Credential Secret <span style={styles.badge}>NET_API key</span></label>
                  <input style={styles.input} type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="Enter NET_API key" />
              ...
              <label style={styles.label}>
                Source Endpoint <span style={{ color: "#9CA3AF", fontSize: "10px", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>(optional — audit trail)</span>
              </label>
              <input style={styles.input} value={apiEndpoint} onChange={e => setApiEndpoint(e.target.value)} placeholder="https://my-sensor.io/readings" />
                ...
                {m1Mode === "random" && (
                  <>
                    ...
                    <div style={{ ...styles.urlPreview, background: "#FFFFFF", color: "#34D399", fontWeight: 600 }}>
                      ...
                      <span style={{ color: "#0E1512" }}>{", \"date\": \"...\", \"time\": \"...\" }"}</span>
                    </div>
                  </>
                )}
                  ...
                  <>
                    <div style={styles.grid2}>
                      ...
                      <div>
                        ...
                        <select style={styles.select} value={sourcePeriod} onChange={e => setSourcePeriod(e.target.value)}>
                          ...
                          <option value="Yearly">Annual Block</option>
                          <option value="Half-yearly">H1/H2 Block</option>
                          <option value="Quarterly">Quarterly Block</option>
                          <option value="Monthly">Monthly Block</option>
                        </select>
                      </div>
                    </div>
                    <div style={styles.grid2}>
                      <div>
                        <label style={styles.label}>Packet Frequency</label>
                      ...
                      {targetFrequency === "Minutes" && (
                        <div>
                          <label style={styles.label}>Step Interval</label>
                    ...
                    <div style={styles.summaryBox}>
                      <div style={styles.summaryItem}>
                        <span style={styles.summaryLabel}>VP Protocol</span>
                    ...
                    <div style={{ ...styles.urlPreview, marginTop: "16px", background: "#FFFFFF", color: "#34D399", fontWeight: 600 }}>
                      Payload: {`{ "value": ${distributions.valuePerInterval.toFixed(4)}, "date": "...", "time": "..." }`}
                    </div>
                  </>
                )}
              </div>
              ...
              <div style={styles.card}>
                ...
                <div style={{ background: "#F9FAF9", borderRadius: "12px", padding: "16px", marginBottom: "24px", border: "1px solid #E6E8E3" }}>
                  <label style={{ ...styles.label, marginBottom: "6px" }}>
                    Formula Expression <span style={{ color: "#9CA3AF", fontSize: "10px", fontWeight: 400, textTransform: "none" }}>(paste to auto-populate variables below)</span>
                  </label>
                ...
                <p style={{ fontSize: "13px", color: "#6B7280", marginBottom: "20px", marginTop: 0 }}>
                  Enter values for each variable. Names must exactly match the formula defined on the project.
                </p>
                ...
                {m2Vars.map((v, i) => (
                  <div key={i} style={styles.rowItem}>
                    <input
                      ...
                      placeholder="Variable name (e.g. NCV)"
                      value={v.key}
                      onChange={e => updateM2Var(i, "key", e.target.value)}
                    />
                    <input
                      ...
                      placeholder="Value"
                      value={v.value}
                      onChange={e => updateM2Var(i, "value", e.target.value)}
                    />
                    {m2Vars.length > 1 && (
                ...
                <div style={{ ...styles.urlPreview, marginTop: "16px", background: "#FFFFFF", color: "#34D399", fontWeight: 600 }}>
                  Payload: {`{ "variables": { ${m2Vars.filter(v => v.key).map(v => `"${v.key}": ${Number(v.value)||0}`).join(", ")} }, "date": "...", "time": "..." }`}
                </div>
              </div>
            )}
              ...
              <div style={styles.card}>
                ...
                <div style={{ ...styles.urlPreview, marginTop: "16px", background: "#FFFFFF", color: "#34D399", fontWeight: 600, whiteSpace: "pre-wrap" }}>
                  {(() => {
                    ...
                    return `Payload: ${JSON.stringify({ entry, date: "...", time: "..." }, null, 2)}`;
                  })()}
                </div>
              </div>
            )}
            ...
            <div style={{ ...styles.flexRow, gap: "20px", paddingBottom: "60px" }}>
              ...
              >
                {status === "RUNNING" ? "Cease Stream" : "Launch Mainframe Stream"}
              </button>
          ...
          <div className="side" style={styles.sidePanelCol}>
            <div style={styles.console}>
              ...
              <div style={{ padding: "20px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", borderBottom: "1px solid #E6E8E3" }}>
                ...
                <div style={{ textAlign: "center" }}>
                  <div style={styles.summaryLabel}>Relay</div>
