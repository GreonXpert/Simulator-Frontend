...
const APIDataSimulator = () => {
  ...
  const fetchScope12Total = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        setScope12Data(result.data); setScope12LastFetch(new Date());
        log(`✓ Baseline Updated: ${result.data.scope12TotalCO2e} tCO₂e`, 'ok');
      } else throw new Error('Invalid format');
    } catch (error) { log(`❌ Error: ${error.message}`, 'err'); }
  ...
  const getCurrentFields = () => {
    ...
    const cat = scopeConfig[selectedScope][selectedCategory];
    if (!cat) return [];
    if (cat.activities && cat.activities.length > 0) return selectedActivity ? (cat.fields[selectedActivity]?.[selectedTier] || []) : [];
    return cat.fields[selectedTier] || [];
  };

  const buildPayload = (overrideIdx = null) => {
    let now = new Date();
  ...
  const startAuto = () => {
    ...
    setStatus("RUNNING");
    log(`Broadcaster Active @ ${ms}ms`, "ok");
    timerRef.current = setInterval(sendOnce, ms);
  };

  const stopAuto = () => { if (timerRef.current) { clearInterval(timerRef.current); timerRef.current = null; setStatus("STOPPED"); log("Broadcaster halted", "warn"); } };
  const resetControlled = () => { setSendsCompleted(0); log("Progress reset", "info"); };
  ...
  const hasActivityField = () => { const c = scopeConfig[selectedScope][selectedCategory]; return c?.activities && c.activities.length > 0; };
          ...
          <div className="main">
            ...
            {simMode === "controlled" && (
              <div style={{...styles.card, background: "#E7FBF2", border: "none"}}>
                 ...
                 <div style={styles.grid}>
                    <div><label style={styles.label}>Packet frequency</label><select style={styles.select} value={targetFrequency} onChange={e=>setTargetFrequency(e.target.value)}>
                      <option value="Monthly">Monthly</option><option value="Daily">Daily</option><option value="Hourly">Hourly</option><option value="Minutes">Minutes</option>
                    </select></div>
                    {targetFrequency === "Minutes" && <div><label style={styles.label}>Step Interval</label><input style={styles.input} type="number" value={minuteInterval} onChange={e=>setMinuteInterval(e.target.value)}/></div>}
                 </div>
                 <div style={styles.summaryBox}>
                    <div style={styles.summaryItem}><span style={styles.summaryLabel}>VP Protocol</span><span style={styles.summaryValue}>{distributions.valuePerInterval.toFixed(4)}</span></div>
            ...
            <div style={styles.card}>
                ...
                <div style={styles.grid}>
                  <div><label style={styles.label}>Scope Ref</label><select style={styles.select} value={selectedScope} onChange={e=>setSelectedScope(e.target.value)}><option value="Scope 1">Scope 1</option><option value="Scope 2">Scope 2</option><option value="Scope 3">Scope 3</option></select></div>
                  <div><label style={styles.label}>Category Ref</label><select style={styles.select} value={selectedCategory} onChange={e=>setSelectedCategory(e.target.value)}><option value="">Choose category</option>{getAvailableCategories().map(cat=><option key={cat} value={cat}>{cat}</option>)}</select></div>
                </div>
                <div style={styles.grid}>
                   {hasActivityField() && <div><label style={styles.label}>Activity Node</label><select style={styles.select} value={selectedActivity} onChange={e=>setSelectedActivity(e.target.value)}><option value="">Choose activity</option>{getAvailableActivities().map(act=><option key={act} value={act}>{act}</option>)}</select></div>}
            ...
            <div style={styles.card}>
              <div style={styles.sectionHeading}>⇥ Gateway Access</div>
              <label style={styles.label}>Base URL endpoint</label><input style={styles.input} value={baseUrl} onChange={e=>setBaseUrl(e.target.value)}/>
              <div style={styles.grid}>
                 <div><label style={styles.label}>Client Reference</label><input style={styles.input} value={clientId} onChange={e=>setClientId(e.target.value)}/></div>
                 <div><label style={styles.label}>Node Reference</label><input style={styles.input} value={nodeId} onChange={e=>setNodeId(e.target.value)}/></div>
              </div>
              <div style={styles.grid}>
                 <div><label style={styles.label}>Scope Descriptor</label><input style={styles.input} value={scopeId} onChange={e=>setScopeId(e.target.value)}/></div>
            ...
            <div style={{...styles.flexRow, gap: "20px", paddingBottom: "60px"}}>
               <button style={{...styles.btnBlack, flex: 1}} onClick={sendOnce}>Initiate Single Pack</button>
               <button style={{...styles.btnMint, flex: 1.5, background: status==="RUNNING"?"#0E1512":"#34D399", color: status==="RUNNING"?"#FFF":"#0E1512"}} onClick={status==="RUNNING"?stopAuto:startAuto}>{status==="RUNNING"?"Cease Stream":"Launch Mainframe Stream"}</button>
               <button style={{...styles.btnOutline, flex: 1}} onClick={clearLog}>Flush Buffer</button>
            </div>
          </div>
          ...
          <div className="side">
             <div style={styles.console}>
                ...
                <div style={{ padding: "20px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", borderBottom: "1px solid #E6E8E3" }}>
                   <div style={{ textAlign: "center" }}><div style={styles.summaryLabel}>Efficiency</div><div style={{ fontSize: "20px", fontWeight: 700, color: "#10B981" }}>{totalSent>0?Math.round((success/totalSent)*100):0}%</div></div>
                   <div style={{ textAlign: "center" }}><div style={styles.summaryLabel}>Relay</div><div style={{ fontSize: "20px", fontWeight: 700, color: lastHttp===200?"#10B981":"#EF4444" }}>{lastHttp}</div></div>
                </div>
